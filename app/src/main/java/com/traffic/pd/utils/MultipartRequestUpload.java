 ...
 */
class MultipartRequestUpload extends Request<String> {
     ...
     * @param params
     */
    public MultipartRequestUpload(String url, Response.ErrorListener errorListener,
    ...
    private void buildMultipartEntityOnly() {
        if (mFileParts != null && mFileParts.size() > 0) {
            for (int i = 0; i < mFileParts.size(); i++) {
                entity.addPart(mFilePartName, new FileBody(mFileParts.get(i)));
            }
            long l = entity.getContentLength();
        }
        try {
